...
class IndexMaker:
        ...
        for invm_lims in self.invms_lims:
            if print_it:
                if invm_lims != invms_lims[0]:
                    print()
                print(f"Running for mass range: {invm_lims[0]:.2f} -- {invm_lims[1]:.2f}")
    ...
    def loop_and_save(self, invm_lo: float, invm_hi: float) -> None:
        ...
        for num_splits in self.num_splits_choices:
            ...
            splits = [(split_lim_inds[i], split_lim_inds[i + 1]) for i in range(num_splits)]
            for split in splits:
    ...
    def choose_inds(self, invm_lo: float, invm_hi: float) -> Sequence[int]:
        ...
        if self.N > len(evt_inds):
            if self.error:
                raise IndexError(
                    f"**** ERROR: Asking for N={N}, " + f"but there are only {len(evt_inds)} events... ****",
                ) from None

            print(
                f"**** WARNING: Asking for N={N}, " + f"but there are only {len(evt_inds)} events... ****",
                end=f"\n{' ' * 4}",
            )
            self.N = len(evt_inds)
