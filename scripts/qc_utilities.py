...
def get_lambdas(
    ...
    match ltype:
        ...
        case "Pijmax":
            ...
            lambdas = max_Pijs
        case _:
            raise Exception(f"`ltype` must be in {LAMBDA_CHOICES} but is {ltype}")
...
def _get_minimum_energies(htype: str, p4s: _p4s, lambdas: Sequence[float], bit_string_indices: _bs_bools):
    ...
    for ind, (p4, lmbda) in enumerate(zip(p4s, lambdas)):
        ...
        min2nd_bitstring_str = "".join(["1" if b else "0" for b in min2nd_bitstring])
...
re_inds_dir = rf"^inds(\d+)_(\w+)_(\w+)_(\d{{3}})x(\d{{3}})_({re_float})to({re_float})$"
...
def get_all_indices(dir_name):
    ...
    ind_fname = list(dir_name.glob(f"*{'1':0>{len(str(N_evts))}}to{N_evts}.npz"))
...
def get_all_depths(etype, dtype, alg, quadcoeff, noise=False, bitflip_prob="\d+"):
    """
...
def get_files(ntype, ftype, noise=False, **kwargs):
    ...
    if ftype == "eff":
        ...
        if ntype == "dir":
            ...
            files = []
            for fdir_name in data_dir.glob(f"*_{err_prob}*/*" if noise else "*/*"):
                if re.match(re_file_name, fdir_name.name) is not None:
                    files.append(data_dir / fdir_name)
            return files
    elif ftype == "inds":
        re_dir_name = f"inds{N_evts}_{etype}_{dtype}_{uid}x\d{{3}}_{invm_lo}to{invm_hi}"
